...
const Footer: FC<FooterProps> = ({ footerData, companyInfo }) => {
  ...
  const tagline = companyInfo?.tagline || "Expert mechanical engineering and rapid prototyping services. We transform product concepts into functional prototypes.";
  ...
  const copyrightText = footerData?.copyrightText || "YK Innovations. All rights reserved.";
          ...
          <div>
            ...
            <ul className="space-y-2">
              {services.map((service, index) => (
                <li key={index}>
                  <Link href={service.href} className="text-base-content/70 hover:text-primary text-sm transition-colors duration-200">
                    {service.label}
                  </Link>
                </li>
              ))}
          ...
          {legalLinks && legalLinks.length > 0 && (
            <div className="flex gap-6 text-sm">
              {legalLinks.map((link, index) => (
                <Link key={index} href={link.href} className="text-base-content/60 hover:text-primary transition-colors duration-200">
                  {link.label}
                </Link>
              ))}
            </div>
